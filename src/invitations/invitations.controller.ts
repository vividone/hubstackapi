import { Controller, Post, Body, NotFoundException, InternalServerErrorException, Req } from '@nestjs/common';
...
export class InvitationsController {
    ...
    @Post('send-invite')
    createInvitation(@Body() createInvitationDto: CreateInvitationDto, @Req() req: Request) {
      const { invitersUsername } = createInvitationDto;

      const superAgent = this.superAgentService.findAgentByUsername(invitersUsername);
      if (!superAgent) {
