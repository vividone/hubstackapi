...
export class MockWalletService {
    ...
    async createCustomerWallet(data: CreateMockWalletDto, id: string) {
        ...
        const { bvn, existingAccountNumber, existingBankName, bankName, accountNumber, ...rest } = data;
        ...
        try {
            ...
            const customer = await this.createCustomer(user._id, baseUrl, secretKey, email, rest);
            const bankCode = await this.getBankCode(baseUrl, secretKey, bankName);
            ...
            const validatePayload = {
                ...
                bvn: bvn,
                bank_code: bankCode,
                account_number: data.existingAccountNumber,
            };

            const validateCustomer = validatePayload;
            ...
            const dvaData = this.createDvaData(customer.customer_id, newAccountNumber);
            ...
            this.handleAxiosError(error, 'An error occurred while creating customer and DVA');
        }
    }

    private async createCustomer(userId: string, baseUrl: string, secretKey: string, email: string, data: any) {
        try {
            const response = await axios.post(`${baseUrl}/customer`, { email, ...data }, {
                headers: {
            ...
            return { email, customer_code, customer_id: id, integration, user: userId };
        } catch (error) {
            this.handleAxiosError(error, 'An error occurred while creating the customer');
        }
    }

    private async getBankCode(baseUrl: string, secretKey: string, bankName: string) {
        try {
            ...
            this.handleAxiosError(error, 'An error occurred while retrieving the bank code');
        }
    }
